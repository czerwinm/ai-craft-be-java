...
public class RestTemplateFixture {
  ...
  public RestTemplateManipulation getRestTemplate(String beanName, String restTemplateFieldName) {
    RestTemplate restTemplate =
        (RestTemplate)
            ReflectionTestUtils.getField(context.getBean(beanName), restTemplateFieldName);
  ...
  public RestTemplateManipulation getRestTemplate(Class<?> beanType, String restTemplateFieldName) {
    RestTemplate restTemplate =
        (RestTemplate)
            ReflectionTestUtils.getField(context.getBean(beanType), restTemplateFieldName);
    return new RestTemplateManipulation(restTemplate);
  }
  ...
  public class RestTemplateManipulation {
    ...
    public RestTemplateManipulation override(
        @NotNull Function<@NotNull UriComponentsBuilder, @NotNull UriComponentsBuilder> function) {
      ClientHttpRequestFactory requestFactory = restTemplate.getRequestFactory();
      ClientHttpRequestFactory overriding =
          new ClientHttpRequestFactory() {
            @Override
            public @NotNull ClientHttpRequest createRequest(
                @NotNull URI uri, @NotNull HttpMethod httpMethod) throws IOException {
              return requestFactory.createRequest(
                  function.apply(UriComponentsBuilder.fromUri(uri)).build().toUri(), httpMethod);
            }
          };
    ...
    public RestTemplateManipulation intercept(ClientHttpRequestInterceptor interceptor) {
      restTemplate.setRequestFactory(
          new InterceptingClientHttpRequestFactory(
              restTemplate.getRequestFactory(), List.of(interceptor)));
      return this;
    }
    ...
    private record MockRestServerInterceptor(ClientHttpRequestFactory mocked)
        implements ClientHttpRequestInterceptor {

      @Override
      public @NotNull ClientHttpResponse intercept(
          HttpRequest request, byte[] body, @NotNull ClientHttpRequestExecution execution)
          throws IOException {
