...
class IoT20ControllerTest {
  ...
  @MockitoBean private CommunicationService service;

  @Test
  void updateAll() throws Exception {
    Mockito.when(service.handleBoot(any()))
        .thenReturn(
            new CommunicationService.BootResponse(
                Instant.parse("2023-06-28T06:15:30.00Z"),
                Duration.ofSeconds(69),
                KnownDevices.State.EXISTING));

    rest.perform(
            post("/protocols/iot20/bootnotification/{deviceId}", "device-id")
                .with(jwt())
    ...
    Mockito.verify(service)
        .handleBoot(CommunicationFixture.boot().deviceId("device-id").protocol(IoT20).build());
  }
}
