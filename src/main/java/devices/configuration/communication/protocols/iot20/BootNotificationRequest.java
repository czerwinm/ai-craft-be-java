...
record BootNotificationRequest(Device device, Reason reason) {

  record Device(
      String serialNumber, String model, Modem modem, String vendorName, String firmwareVersion) {}
  ...
  BootNotification toBootNotificationEvent(String deviceId) {
    return new BootNotification(
        deviceId,
        BootNotification.Protocols.IoT20,
        device.vendorName,
        device.model,
        device.serialNumber,
